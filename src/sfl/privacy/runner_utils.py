...
def add_privacy_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--ffl-root-update", type=str, default=None,
                        help="Path to .npy file with root update vector for FoundationFL. "
                             "Required for secure mode — provides a trusted reference that "
                             "prevents Byzantine majority attacks")
...
def build_privacy_mods(args: argparse.Namespace) -> List[Any]:
    ...
    if args.aggregation == "krum":
        ...
        if getattr(args, "ffl_root_update", None):
            os.environ["SFL_FFL_ROOT_UPDATE"] = args.ffl_root_update
